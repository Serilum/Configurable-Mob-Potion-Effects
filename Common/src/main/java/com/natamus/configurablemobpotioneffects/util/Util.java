...
public class Util {
	...
	private static CopyOnWriteArrayList<MobEffectInstance> parseEffectString(String effectstring) {
		...
		for (String effectpair : effectstring.split(StringFunctions.escapeSpecialRegexChars("|"))) {
			...
			MobEffect effect = BuiltInRegistries.MOB_EFFECT.get(ResourceLocation.parse(effectrlstring));
			if (effect == null) {
				continue;
			}
			...
			MobEffectInstance instance = new MobEffectInstance(BuiltInRegistries.MOB_EFFECT.wrapAsHolder(effect), duration*20, level-1, true, true);
