...
public class Util {
	...
	public static void loadMobConfigFile() throws IOException {
		...
		if (!dir.isDirectory() || !permanentfile.isFile() || !damagefile.isFile()) {
			...
			for (String line : permanentcontent.split("\n")) {
				...
				Optional<Holder.Reference<EntityType<?>>> entitytypeOptionalReference = BuiltInRegistries.ENTITY_TYPE.get(Identifier.parse(entityrl));
		...
		if (permanentwriter != null || damagewriter != null) {
			for (Identifier loc : BuiltInRegistries.MOB_EFFECT.keySet()) {
			...
			for (Identifier loc : BuiltInRegistries.ENTITY_TYPE.keySet()) {
			...
			for (String effectstring : sortedpotions) {
				...
				Identifier rl = BuiltInRegistries.MOB_EFFECT.getKey(effect);
	...
	private static CopyOnWriteArrayList<MobEffectInstance> parseEffectString(String effectstring) {
		...
		for (String effectpair : effectstring.split(StringFunctions.escapeSpecialRegexChars("|"))) {
			...
			Optional<Holder.Reference<MobEffect>> mobEffectOptionalReference = BuiltInRegistries.MOB_EFFECT.get(Identifier.parse(effectrlstring));
			if (mobEffectOptionalReference.isEmpty()) {
				continue;
			}
